...
class AmazonScraper {
    ...
    async startScraper() {
        ...
        if (this.scrapeType === 'questions') {
            this.asyncPage = Math.ceil(this.number / 10);
            if (!this.asin) {
                throw new Error('ASIN is missing');
            }
            if (this.number > CONST.limit.questions) {
                throw new Error(`Wow.... slow down cowboy. Maximum you can get is ${CONST.limit.questions} questions`);
            }
        }
    ...
    get setRequestEndpoint() {
        switch (this.scrapeType) {
            ...
            case 'questions':
                return `ask/questions/asin/${this.asin}/ref=ask_ql_psf_ql_hza`;
            case 'asin':
    ...
    async buildRequest(page) {
        ...
        if ((this.scrapeType === 'questions') && (page > 1)) {
            options.uri = options.uri.replace('/ref=ask_ql_psf_ql_hza', `/${page}/ref=ask_ql_psf_ql_hza`);
        }
    ...
    grabQuestions(body) {
        ...
        const quetionsList = $('.askTeaserQuestions').children();
        let scrapingResult = {};
        for (let i = 0; i < quetionsList.length; i++) {
            const totalInResult = Object.keys(scrapingResult).length + this.collector.length;
            if (totalInResult >= this.number && this.bulk) {
                break;
            }
            const questionId = quetionsList[i].children[0].children[1].children[0].attribs['id'].replace("question-", "");
            if (!questionId) {
                continue;
            }

            scrapingResult[questionId] = { id: questionId };

            const questionLocation = quetionsList[i].children[0].children[1].children[1].children[0].children[1];
            if (questionLocation.children.length > 1) {
                let questionAnswer = questionLocation.children[2];
                if (Object.keys(questionAnswer.attribs).length > 0) {
                    questionAnswer = $(questionAnswer.children[2]).text();
                } else {
                    questionAnswer = $(questionAnswer).text();
                }
                scrapingResult[questionId].answer = questionAnswer;
            }
        }
        ...
        for (let key in scrapingResult) {
            const search = $(`#question-${key} [data-action="ask-no-op"]`);
            ...
            scrapingResult[key].question = search[0].children[0].data;
        }
